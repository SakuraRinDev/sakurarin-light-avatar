...
- MCP: `/api/mcp` exposes an MCP-compatible internal tool registry from `data/mcp-servers.json`; `/api/mcp?manifest=1` returns a tool manifest, and chat replies include matched `mcp` tool candidates.
- Location: the `LOC` button uses the browser Geolocation API only after user permission. Rounded coordinates are sent to `/api/dialogue` only for location-relevant messages and stored with reduced precision.
